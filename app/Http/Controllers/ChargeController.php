...
public function relance(Charge $charge)
{
    $occupant = $charge->appartement->occupants()->first();

    if (!$occupant || !$occupant->email) {
        return back()->with('error', 'Cet occupant n\'a pas d\'email enregistré !');
    }

    Mail::to($occupant->email)->send(new RelanceImpaye($occupant, $charge));

    return back()->with('success', "Email de relance envoyé à {$occupant->nom} !");
}
}